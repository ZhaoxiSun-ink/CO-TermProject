...
int calculatev0(int v1, int v2, std::string operater) {
	...
	if (operater.compare("and") == 0 or operater.compare("andi") == 0) {
		return v1 & v2;
	}
	if (operater.compare("or") == 0 or operater.compare("ori") == 0) {
		return v1 ^ v2;
	}
	if (operater.compare("slt") == 0 or operater.compare("slti") == 0) {
		return (v1 < v2) ? 1 : 0;
	}
	return 0;
}
...
void readInstruction(string& target, const string instruction, string& reg1, string& reg2, char type) {
	if (type == 'N') {
		int regCount = 0;
		int i = 0;
		while ((unsigned)i < instruction.length()) {
			if (instruction[i] == '$') {
				switch (regCount) {
				case 0:
					regCount++;
...
int main(int argc, const char* argv[]) {
	...
	if (string(argv[1]).compare("F") == 0) {
		...
		while (i < 16) {
			int j = 0;
			while ((unsigned)j <= cycleInstru.size()) {
				std::string instruCurr;
				std::string operation;
				std::string instruType;
				if (j == i) {
					if (cycleInstru.size() != (unsigned)j + 1) {
						if ((unsigned)nextInstru >= allInstru.size()) {
							break;
						}
				...
				if ((unsigned)j == cycleInstru.size()) {
					break;
				}
				...
				if ((unsigned)j == cycleInstru.size() - 1 and stageCurr == 5) {
					...
					if (stageCurr == 3) {
						...
						if (nop) {
							if (nop == 1) {
								...
								for (int n = j; (unsigned)n < cycleInstru.size(); n++) {
